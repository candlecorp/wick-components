...
impl UnzipComponent {
    ...
    fn unzip_wrapper(input: IncomingMono) -> Result<OutgoingMono, GenericError> {
      ...
      let task = async move {
        ...
        #[allow(unused)]
        fn des(mut map: std::collections::BTreeMap<String, Value>) -> Result<unzip_service::unzip::Inputs, Error> {
          Ok(unzip_service::unzip::Inputs {
            source: <String as serde::Deserialize>::deserialize(map.remove("source").ok_or_else(|| wasmrs_guest::Error::MissingInput("source".to_owned()))?).map_err(|e| wasmrs_guest::Error::Decode(e.to_string()))?,
          })
        }
...
//) -> wasmrs_guest::Flux<read::Outputs, PayloadError> {
  ...
  Host::default().request_stream(payload).map(|result| {
      result
          .map(|payload| Ok(deserialize::<read::Outputs>(&payload.data.unwrap())?))?
  })
}
...
//) -> wasmrs_guest::Flux<write::Outputs, PayloadError> {
  ...
  let first = OpInputs::Params(write::InputFirst {
    dest: inputs.dest,
  });
  ...
  Host::default().request_channel(rx).map(|result| {
      result
          .map(|payload| Ok(deserialize::<write::Outputs>(&payload.data.unwrap())?))?
  })
}
...
pub(crate) fn init_imports() {
  ...
  wasmrs_guest::add_import(
    u32::from_be_bytes(READER_READ_INDEX_BYTES),OperationType::RequestStream,"unzip.Reader","read",
  );

  wasmrs_guest::add_import(
    u32::from_be_bytes(WRITER_WRITE_INDEX_BYTES),OperationType::RequestChannel,"unzip.Writer","write",
  );
}
pub(crate) fn init_exports() {
  
    wasmrs_guest::register_request_response(
      "unzip.Unzip","unzip",UnzipComponent::unzip_wrapper,
    );
